...
test('comments: owner can post and list; stranger cannot list', async () => {
  ...
  const m = await prisma.memory.create({
    data: { user_id: owner.id, source: 't', title: 't', content: 'x', memory_type: 'LOG_EVENT', confidence_score: 0.5, timestamp: BigInt(Date.now()) },
  })
...
test('comments: shared user can comment with COMMENT permission', async () => {
  ...
  await createShare({ memoryId: m.id, sharerUserId: owner.id, recipientType: 'USER', recipientUserId: shared.id, permission: 'COMMENT' })
...
test('comments: edit own works, delete own marks deleted_at', async () => {
  ...
  const m = await prisma.memory.create({
    data: { user_id: u.id, source: 't', title: 't', content: 'x', memory_type: 'LOG_EVENT', confidence_score: 0.5, timestamp: BigInt(Date.now()) },
  })
