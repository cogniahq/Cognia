...
test('crud: listMemories with organizationId=null scopes to personal vault', async () => {
  const u = await prisma.user.create({ data: { email: `pv-${randomUUID()}@x.io` } })
  const org = await prisma.organization.create({
    data: { name: `org-${randomUUID()}`, slug: `org-${randomUUID()}` },
  })
  const personal = await makeMemory(u.id, 'p')
  ...
  const list = await listMemories({ userId: u.id, organizationId: null })
  assert.equal(list.items.length, 1)
  assert.equal(list.items[0].id, personal.id)
})

test('crud: listMemories with organizationId=<orgId> scopes to that org', async () => {
  const u = await prisma.user.create({ data: { email: `os-${randomUUID()}@x.io` } })
  const org = await prisma.organization.create({
    data: { name: `org-${randomUUID()}`, slug: `org-${randomUUID()}` },
  })
  await makeMemory(u.id, 'p')
  const orgMem = await prisma.memory.create({
    data: {
      user_id: u.id,
      organization_id: org.id,
      source: 'test',
  ...
  const list = await listMemories({ userId: u.id, organizationId: org.id })
  assert.equal(list.items.length, 1)
  assert.equal(list.items[0].id, orgMem.id)
})
