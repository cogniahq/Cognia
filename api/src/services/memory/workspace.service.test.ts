...
test('workspace: create + list + slug uniqueness', async () => {
  ...
  const o = await prisma.organization.create({ data: { name: `t-${randomUUID()}`, slug: `t-${randomUUID()}` } })
...
test('workspace: moveMemory assigns workspace_id', async () => {
  ...
  const o = await prisma.organization.create({ data: { name: `m-${randomUUID()}`, slug: `m-${randomUUID()}` } })
  const w = await createWorkspace(o.id, 'A', u.id)
  const m = await prisma.memory.create({
    data: { user_id: u.id, organization_id: o.id, source: 't', content: 'x', memory_type: 'LOG_EVENT', confidence_score: 0.5, timestamp: BigInt(Date.now()) },
  })
