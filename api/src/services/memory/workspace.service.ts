...
export async function createWorkspace(orgId: string, name: string, createdByUserId: string, description?: string) {
  const baseSlug = slugify(name)
  let slug = baseSlug
  let i = 0
  while (await prisma.workspace.findUnique({ where: { organization_id_slug: { organization_id: orgId, slug } } })) {
    i++; slug = `${baseSlug}-${i}`
  }
...
export async function listWorkspaces(orgId: string) {
  return prisma.workspace.findMany({ where: { organization_id: orgId }, orderBy: { created_at: 'asc' } })
}
...
export async function moveMemoryToWorkspace(memoryId: string, userId: string, workspaceId: string | null) {
