  ...
  return (
    ...
    >
      <Section id="encryption" title="Encryption">
        <p>
          All traffic between your browser, our APIs, and our backing
          databases is encrypted in transit with TLS 1.2 or higher; older
          protocols are explicitly disabled.
        </p>
        <p>
          At rest, we apply AES-256-GCM column-level encryption to OAuth
          access and refresh tokens, TOTP 2FA secrets, and OIDC client
          secrets. Backing volumes are additionally encrypted by the
        ...
        <p>
          ...
          KMS (AWS KMS) with envelope encryption and per-tenant data keys;
          the rotation tooling already lives behind the same interface.
        </p>
      </Section>
      ...
      <Section id="access-controls" title="Access controls">
        <ul className="list-disc pl-6 space-y-1">
          <li>
            Role-based access control with three tiers — ADMIN, EDITOR,
            VIEWER — enforced at the API layer.
          </li>
          <li>
            JWT access tokens are short-lived; refresh tokens rotate on
            every use with reuse-detection that revokes the entire token
            family on suspicious replay.
          </li>
          <li>
            Configurable session timeout per organization, with an
            org-admin-controlled IP allowlist that rejects connections at
            the gateway.
          </li>
          <li>
            Mandatory 2FA can be enforced for all org members; admins
            cannot disable their own 2FA without re-verification.
          </li>
        </ul>
      </Section>
      ...
      <Section id="audit-logging" title="Audit logging">
        ...
        <p>
          ...
          Retention is configurable per organization at 90 days, 365 days,
      ...
      <Section id="network" title="Network">
        <ul className="list-disc pl-6 space-y-1">
          <li>
            HTTP responses include a strict set of security headers via
            Helmet — HSTS with preload, a CSP that blocks inline scripts
            outside an explicit nonce list, X-Frame-Options DENY, and
          ...
          <li>
            Per-IP and per-user rate limits protect login, password reset,
            and write endpoints. Failed-auth events feed an automatic
            temporary lockout.
          </li>
        </ul>
      </Section>
      ...
      <Section id="compliance" title="Compliance">
        <ul className="list-disc pl-6 space-y-1">
          <li>
            SOC 2 Type 1 audit is in progress with a target completion of
            Q3. The audit letter will be available under NDA via the trust
            center.
          </li>
          <li>
            GDPR-ready: data subject rights (access, rectification,
            erasure, portability) are exposed in-product and by request.
          </li>
          <li>
      ...
      <Section id="incident-response" title="Incident response">
        <p>
          ...
          incidents, we publish a public root-cause analysis within 72
          hours of resolution and notify affected customers within 24 hours
          of confirming any exposure of personally identifiable
          information.
        </p>
      </Section>
