...
/**
 * Top-of-page banner shown when the org's subscription is in a dunning
 * state. For Razorpay this maps to `halted` (auto-charge has failed and
 * Razorpay has stopped retrying) and `pending`. Clicking the CTA re-runs
 * the Razorpay Checkout flow so the user can re-authorise payment.
 */
export const DunningBanner: React.FC<DunningBannerProps> = ({
  ...
  const showBanner =
    status === "halted" ||
    status === "pending" ||
    // Legacy Stripe statuses kept here so a transitional state is still
    // surfaced.
    status === "past_due" ||
    status === "unpaid" ||
    status === "incomplete"
